/*
 * $Id: JXMonthView.java,v 1.14 2005/10/27 19:35:52 dmouse Exp $
 * 
...
/**
 * Component that displays a month calendar which can be used to select a day or range of days. By
 * default the <code>JXMonthView</code> will display a single calendar using the current month and
 * year, using <code>Calendar.SUNDAY</code> as the first day of the week.
 * <p>
 * The <code>JXMonthView</code> can be configured to display more than one calendar at a time by
 * calling <code>setPreferredCalCols</code>/<code>setPreferredCalRows</code>. These methods
 * will set the preferred number of calendars to use in each column/row. As these values change, the
 * <code>Dimension</code> returned from <code>getMinimumSize</code> and
 * <code>getPreferredSize</code> will be updated. The following example shows how to create a 2x2
 * view which is contained within a <code>JFrame</code>:
 * 
 * <pre>
 ...
 * 
 * <p>
 * <code>JXMonthView</code> can be further configured to allow any day of the week to be
 * considered the first day of the week. Character representation of those days may also be set by
 ...
 * 
 * <p>
 * This component supports flagging days. These flagged days are displayed in a bold font. This can
 * be used to inform the user of such things as scheduled appointment.
 * 
 * <pre>
 ...
 * cal2.set(2004, 1, 5);
 * 
 * long[] flaggedDates = new long[] { cal1.getTimeInMillis(), cal2.getTimeInMillis(), System.currentTimeMillis() };
 * 
 * monthView.setFlaggedDates(flaggedDates);
 * </pre>
 * 
 * Applications may have the need to allow users to select different ranges of dates. There are four
 * modes of selection that are supported, single, multiple, week and no selection. Once a selection
 * is made an action is fired, with exception of the no selection mode, to inform listeners that
...
public class JXMonthView extends JComponent {
    ...
    /**
     * Keeps track of the first date we are displaying. We use this as a restore point for the
     * calendar.
     */
    ...
    /**
     * Create a new instance of the <code>JXMonthView</code> class using the month and year of the
     * current day as the first date to display.
     */
    public JXMonthView() {
    ...
    /**
     * Create a new instance of the <code>JXMonthView</code> class using the month and year from
     * <code>initialTime</code> as the first date to display.
     * 
    ...
    public void updateUI() {
        ...
        _monthDownImage = new ImageIcon(JXMonthView.class.getResource(imageLocation));

        imageLocation = UIManager.getString("JXMonthView.monthUpFileName");
        if (imageLocation == null) {
            imageLocation = "resources/month-up.png";
        }
        _monthUpImage = new ImageIcon(JXMonthView.class.getResource(imageLocation));
    }
    ...
    /**
     * Set the first displayed date. We only use the month and year of this date. The
     * <code>Calendar.DAY_OF_MONTH</code> field is reset to 1 and all other fields, with exception
     * of the year and month , are reset to 0.
     * 
    ...
    /**
     * Returns the last date able to be displayed. For example, if the last visible month was April
     * the time returned would be April 30, 23:59:59.
     * 
    ...
    /**
     * Moves the <code>date</code> into the visible region of the calendar. If the date is greater
     * than the last visible date it will become the last visible date. While if it is less than the
     * first visible date it will become the first visible date.
     * 
    ...
    public DateSpan getSelectedDateSpan() {
        ...
        if (_startSelectedDate != -1) {
            result = new DateSpan(new Date(_startSelectedDate), new Date(_endSelectedDate));
        }
        return result;
    }

    /**
     * Selects the dates in the DateSpan. This method will not change the initial date displayed so
     * the caller must update this if necessary. If we are in SINGLE_SELECTION mode only the start
     * time from the DateSpan will be used. If we are in WEEK_SELECTION mode the span will be
     * modified to be valid if necessary.
     * 
     * @param dateSpan DateSpan defining the selected dates. Passing <code>null</code> will clear
     *        the selection.
     */
    ...
    public void setSelectionMode(int mode) throws IllegalArgumentException {
        if (mode != SINGLE_SELECTION && mode != MULTIPLE_SELECTION && mode != WEEK_SELECTION && mode != NO_SELECTION) {
    ...
    protected boolean isFlaggedDate(long date) {
        ...
        if (_flaggedDates != null) {
            result = _flaggedDates.contains(new Long(date));
        }
        return result;
    }
    ...
    public void setFlaggedDates(long[] flaggedDates) {
        if (_flaggedDates == null) {
            ...
            for (int i = 0; i < flaggedDates.length; i++) {
                long flaggedDate = flaggedDates[i];
                ...
                _flaggedDates.add(new Long(_cal.getTimeInMillis()));
            }
    ...
    /**
     * Sets the number of pixels used to pad the left and right side of a day. The padding is
     * applied to both sides of the days. Therefore, if you used the padding value of 3, the number
     * of pixels between any two days would be 6.
     */
    ...
    /**
     * Sets the number of pixels used to pad the top and bottom of a day. The padding is applied to
     * both the top and bottom of a day. Therefore, if you used the padding value of 3, the number
     * of pixels between any two days would be 6.
     */
    ...
    /**
     * Set whether or not the month view will display buttons to allow the user to traverse to
     * previous or next months.
     * 
     * @param traversable set to true to enable month traversing, false otherwise.
     */
    ...
    /**
     * Sets the single character representation for each day of the week. For this method the first
     * days of the week days[0] is assumed to be <code>Calendar.SUNDAY</code>.
     * 
    ...
    /**
     * Gets what the first day of the week is; e.g., <code>Calendar.SUNDAY</code> in the U.S.,
    ...
    /**
     * Sets what the first day of the week is; e.g., <code>Calendar.SUNDAY</code> in US,
     * <code>Calendar.MONDAY</code> in France.
     * 
     ...
     * 
     * @return boolean <code>true</code> if anti-aliased text is enabled, <code>false</code>
     *         otherwise.
     */
     ...
     * 
     * @param antiAlias <code>true</code> for anti-aliased text, <code>false</code> to turn it
     *        off.
     */
    ...
    /**
     * public void setDropShadowMask(int mask) { _dropShadowMask = mask; repaint(); }
     */
    ...
    /**
     * Sets the selected background color to <code>c</code>. The default color is
     * <code>138, 173, 209 (Blue-ish)</code>
     * 
    ...
    /**
     * Sets the color used to draw the bounding box around today. The default is the background of
     * the <code>JXMonthView</code> component.
     * 
    ...
    /**
     * Sets the color used to draw the background of the month string. The default is
     * <code>138, 173, 209 (Blue-ish)</code>.
     * 
     * @param c color to set
     */
    ...
    /**
     * Sets the color used to draw the foreground of the month string. The default is
    ...
    /**
     * Set the color to be used for painting the specified day of the week. Acceptable values are
    ...
    public void setDayForeground(int dayOfWeek, Color c) {
        _dayToColorTable.put(new Integer(dayOfWeek), c);
    }
     ...
     * @return The color to be used for painting the numeric day of the week. If this was no color
     *         has yet been defined the component foreground color will be returned.
     */
    public Color getDayForeground(int dayOfWeek) {
        Color c;
        c = (Color) _dayToColorTable.get(new Integer(dayOfWeek));
        if (c == null) {
            c = getForeground();
        }
        return c;
    ...
    /**
     * Convenience method so subclasses can get the currently painted day's day of the week. It is
     * assumed the calendar, _cal, is already set to the correct day.
     * 
     * @see java.util.Calendar
    ...
    private void update() {
        ...
        for (int i = 0; i < _daysOfTheWeek.length; i++) {
            String dayOfTheWeek = _daysOfTheWeek[i];
        ...
        if (_traversable) {
            int newHeight = _monthDownImage.getIconHeight() + _arrowPaddingY + _arrowPaddingY;
        ...
        if (_dim.width < longestMonthWidth) {
            ...
            if (_traversable) {
                diff += _monthDownImage.getIconWidth() + _monthUpImage.getIconWidth() + (_arrowPaddingX * 4);
            }
        ...
        _calendarHeight = ((_boxPaddingY + _boxHeight + _boxPaddingY) * 7) + (_boxPaddingY + _monthBoxHeight + _boxPaddingY);
        ...
        if (_startSelectedDate != -1 || _endSelectedDate != -1) {
            if (_startSelectedDate > _lastDisplayedDate || _startSelectedDate < _firstDisplayedDate) {
    ...
    /**
     * Sets the border of this component. The Border object is responsible for defining the insets
     * for the component (overriding any insets set directly on the component) and for optionally
     * rendering any border decorations within the bounds of those insets. Borders should be used
     * (rather than insets) for creating both decorative and non-decorative (such as margins and
     * padding) regions for a swing component. Compound borders can be used to nest multiple borders
     * within a single component.
     * <p>
     * As the border may modify the bounds of the component, setting the border may result in a
     * reduced number of displayed calendars.
     * 
     * @param border Border.
     */
     ...
     * and y, and the new size is specified by width and height.
     * 
    ...
    /**
     * Moves and resizes this component to conform to the new bounding rectangle r. This component's
     * new position is specified by r.x and r.y, and its new size is specified by r.width and
     * r.height
     * 
    ...
    /**
     * Sets the language-sensitive orientation that is to be used to order the elements or text
     * within this component. Language-sensitive LayoutManager and Component subclasses will use
     ...
     * indicating that it has not been specified explicitly. The UNKNOWN orientation behaves the
     * same as ComponentOrientation.LEFT_TO_RIGHT.
     * 
     ...
     * 
     * @param font The font to become this component's font; if this parameter is null then this
    ...
    public void addNotify() {
        ...
        secondsTillTomorrow = secondsTillTomorrow - (_cal.get(Calendar.HOUR_OF_DAY) * 3600) - (_cal.get(Calendar.MINUTE) * 60) - _cal.get(Calendar.SECOND);
    ...
    protected void paintComponent(Graphics g) {
        ...
        shadowColor = new Color(shadowColor.getRed(), shadowColor.getGreen(), shadowColor.getBlue(), (int) (.20 * 255));
            ...
            for (int column = 0; column < _numCalCols; column++) {
                ...
                if (_bounds.intersects(clip)) {
                    ...
                    if (_traversable) {
                        ...
                        tmpY = _bounds.y + (_bounds.height - _monthDownImage.getIconHeight()) / 2;
                        g.drawImage(_monthDownImage.getImage(), tmpX, tmpY, null);

                        tmpX = _bounds.x + _bounds.width - _arrowPaddingX - _monthUpImage.getIconWidth();
                    ...
                    tmpX = _ltr ? x + (_calendarWidth / 2) - (fm.stringWidth(monthName) / 2) : x - (_calendarWidth / 2) - (fm.stringWidth(monthName) / 2) - 1;
                    tmpY = _bounds.y + ((_monthBoxHeight - _boxHeight) / 2) + fm.getAscent();
                ...
                _bounds.y = y + _boxPaddingY + _monthBoxHeight + _boxPaddingY + _boxPaddingY;
                ...
                if (_bounds.intersects(clip)) {
                    ...
                    for (int i = 0; i < DAYS_IN_WEEK; i++) {
                        tmpX = _ltr ? x + (i * (_boxPaddingX + _boxWidth + _boxPaddingX)) + _boxPaddingX + (_boxWidth / 2) - (fm.stringWidth(_daysOfTheWeek[dayIndex]) / 2) : x
                                - (i * (_boxPaddingX + _boxWidth + _boxPaddingX)) - _boxPaddingX - (_boxWidth / 2) - (fm.stringWidth(_daysOfTheWeek[dayIndex]) / 2);
                        tmpY = _bounds.y + fm.getAscent();
                        if ((_dropShadowMask & WEEK_DROP_SHADOW) != 0) {
                            g.setColor(shadowColor);
                            g.drawString(_daysOfTheWeek[dayIndex], tmpX + 1, tmpY + 1);
                            g.setColor(getDaysOfTheWeekForeground());
                        }
                ...
                _bounds.x = _startX + (_ltr ? column * (_calendarWidth + CALENDAR_SPACING) : -(column * (_calendarWidth + CALENDAR_SPACING) + _calendarWidth));
                _bounds.y = _startY + row * (_calendarHeight + CALENDAR_SPACING);
                _bounds.width = _calendarWidth;
                _bounds.height = _calendarHeight;
    ...
    /**
     * Paints a month. It is assumed the calendar, _cal, is already set to the first day of the
     * month to be painted.
     * 
    ...
    private void paintMonth(Graphics g) {
        ...
        for (int i = 0; i < days; i++) {
            ...
            if (_bounds.intersects(clip)) {
                ...
                paintDayBackground(g, _bounds.x, _bounds.y, _bounds.width, _bounds.height, day);
                paintDayForeground(g, _bounds.x, _bounds.y, _bounds.width, _bounds.height, day);
            }
            _cal.add(Calendar.DAY_OF_MONTH, 1);
        }
    }
    ...
    /**
     * Paints the background of the month string. The bounding box for this background can be
     * modified by setting its insets via setMonthStringInsets. The color of the background can be
     * set via setMonthStringBackground.
     * 
    ...
    protected void paintDayForeground(Graphics g, int x, int y, int width, int height, long date) {
        String numericDay = _dayOfMonthFormatter.format(new Date(date));
        FontMetrics fm;
        ...
        if (isFlaggedDate(date)) {
            ...
            g.setFont(_derivedFont);
            fm = getFontMetrics(_derivedFont);
            g.drawString(numericDay, _ltr ? x + _boxPaddingX + _boxWidth - fm.stringWidth(numericDay) : x + _boxPaddingX + _boxWidth - fm.stringWidth(numericDay) - 1, y + _boxPaddingY
                    + fm.getAscent());
            g.setFont(oldFont);
        } else {
            fm = g.getFontMetrics();
    ...
    /**
     * Returns true if the specified date falls within the _startSelectedDate and _endSelectedDate
     * range.
     */
    ...
    /**
     * Calculates the _numCalCols/_numCalRows that determine the number of calendars that can be
     * displayed.
     */
    private void calculateNumDisplayedCals() {
        ...
        _numCalCols = 1;
        _numCalCols += (getWidth() - _calendarWidth) / (_calendarWidth + CALENDAR_SPACING);

        // Determine how many rows of calendars we want to paint.
        _numCalRows = 1;
        _numCalRows += (getHeight() - _calendarHeight) / (_calendarHeight + CALENDAR_SPACING);
    ...
    /**
     * Calculates the _startX/_startY position for centering the calendars within the available
     * space.
     */
    ...
    /**
     * Calculate the bounding box for drawing a date. It is assumed that the calendar, _cal, is
     * already set to the date you want to find the offset for.
     * 
    ...
    private void calculateBoundsForDay(Rectangle bounds) {
        ...
        int diffMonths = month - _firstDisplayedMonth + ((year - _firstDisplayedYear) * MONTHS_IN_YEAR);
        ...
        bounds.x = _ltr ? bounds.x * (_boxPaddingX + _boxWidth + _boxPaddingX) : (bounds.x + 1) * (_boxPaddingX + _boxWidth + _boxPaddingX);
        ...
        bounds.y = _boxPaddingY + _monthBoxHeight + _boxPaddingY + +_boxPaddingY + _boxHeight + _boxPaddingY;
    ...
    /**
     * Return a long representing the date at the specified x/y position. The date returned will
     * have a valid day, month and year. Other fields such as hour, minute, second and milli-second
     * will be set to 0.
     * 
     * @param x X position
     * @param y Y position
    ...
    public long getDayAt(int x, int y) {
        ...
        int calCol = (_ltr ? (x - _startX) : (_startX - x)) / (_calendarWidth + CALENDAR_SPACING);
        ...
        int row = 1;
        row += (((y - _startY) - (calRow * (_calendarHeight + CALENDAR_SPACING))) - (_boxPaddingY + _monthBoxHeight + _boxPaddingY)) / (_boxPaddingY + _boxHeight + _boxPaddingY);
        ...
        int col = ((_ltr ? (x - _startX) : (_startX - x)) - (calCol * (_calendarWidth + CALENDAR_SPACING))) / (_boxPaddingX + _boxWidth + _boxPaddingX);
        ...
        if (daysToAdd < 0 || daysToAdd > (_cal.getActualMaximum(Calendar.DAY_OF_MONTH) - 1)) {
            return -1;
        }
     ...
     * This method should only be called when <code>setTraversable</code> is set to true.
     * 
    ...
    protected int getTraversableButtonAt(int x, int y) {
        ...
        y = ((y - _startY) - (calRow * (_calendarHeight + CALENDAR_SPACING))) - _boxPaddingY;
        ...
        if (x > _arrowPaddingX && x < (_arrowPaddingX + _monthDownImage.getIconWidth() + _arrowPaddingX)) {
            return MONTH_DOWN;
        }

        if (x > (_calendarWidth - _arrowPaddingX * 2 - _monthUpImage.getIconWidth()) && x < (_calendarWidth - _arrowPaddingX)) {
            return MONTH_UP;
        }
        return -1;
    }
    ...
    protected void fireActionPerformed() {
        ...
        for (int i = listeners.length - 2; i >= 0; i -= 2) {
            if (listeners[i] == ActionListener.class) {
                if (e == null) {
                    e = new ActionEvent(JXMonthView.this, ActionEvent.ACTION_PERFORMED, _actionCommand);
                }
     ...
     * @param l The ActionListener that is to be notified
     */
    public void addMouseListener(JMonthViewListener l) {
        listenerList.add(JMonthViewListener.class, l);
    }
     ...
     * @param l The action listener to remove.
     */
    public void removeMouseListener(JMonthViewListener l) {
        listenerList.remove(JMonthViewListener.class, l);
    }
    ...
    protected void fireDoubleClickOnDayPerformed() {
        JMonthViewListener[] listeners = listenerList.getListeners(JMonthViewListener.class);
        for (JMonthViewListener monthViewListener : listeners) {
            monthViewListener.doubleClickOnDayPerformed();
        }
    ...
    protected void processMouseEvent(MouseEvent e) {
        ...
        if (id == MouseEvent.MOUSE_PRESSED) {
            ...
            if (_selectionMode == MULTIPLE_SELECTION || _selectionMode == WEEK_SELECTION) {
                _pivotDate = selected;
            }
    ...
    private class KeyboardAction extends AbstractAction {
        ...
        public void actionPerformed(ActionEvent ev) {
            ...
            // TODO: Modify this to allow keyboard selection even if we don't have a previous
            // selection.
            if (_startSelectedDate != -1 && selectionMode != NO_SELECTION) {
                ...
                if (action >= ACCEPT_SELECTION && action <= CANCEL_SELECTION && _usingKeyboard) {
                    ...
                    _usingKeyboard = true;
                    traverse(action);
                } else if (selectionMode >= MULTIPLE_SELECTION && action >= ADD_PREVIOUS_DAY && action <= ADD_TO_NEXT_WEEK) {
                    _usingKeyboard = true;
        ...
        /**
         * If we are in a mode that allows for range selection this method will extend the currently
         * selected range.
         * 
         * NOTE: This may not be the expected behavior for the keyboard controls and we ay need to
         * update this code to act in a way that people expect.
         */
